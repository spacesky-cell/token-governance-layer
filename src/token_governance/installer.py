...
def discover_stable_commands(
    ...
    for key, command_name, script_name in (
        ...
        if os.name == "nt":
            ...
            if _same_path(expected_root, PACKAGE_ROOT) is False:
                raise InstallError(GLOBAL_INSTALL_GUIDANCE)
        else:
            if (
...
def install_project(
    ...
    *,
    ...
    if server is not None and not _is_owned_mcp(server, paths["config"]):
        raise InstallError(
    ...
    has_owned = server is not None or _has_owned_hook(
        settings, paths["config"], paths["state"]
    )
    incomplete = paths["state"].exists() or has_owned
...
def uninstall_project(
    ...
    *,
    ...
    updated_settings, hook_removed = _without_owned_hooks(
        settings,
        config_path=paths["config"],
        state_path=paths["state"],
    )
    ...
    if isinstance(servers, dict):
        ...
        if isinstance(server, dict) and _is_owned_mcp(server, paths["config"]):
...
def doctor_project(
    ...
    *,
    ...
    hook_command = _owned_hook_command(
        settings, paths["config"], paths["state"]
    )
    server = _mcp_server(mcp_config)
    ownership_ok = hook_command is not None and server is not None and _is_owned_mcp(
        server, paths["config"]
    )
    command_paths = []
    if hook_command is not None:
        command_paths.append(Path(shlex.split(hook_command, posix=True)[0]))
    if isinstance(server, dict) and isinstance(server.get("command"), str):
        command_paths.append(Path(server["command"]))
...
def _is_owned_mcp(server: dict[str, Any], config_path: Path) -> bool:
    return (
    ...
    *,
    ...
    for entry in post_tool_use:
        ...
        for hook in entry["hooks"]:
            ...
            if _is_owned_hook_command(command, config_path, state_path):
                removed = True
            else:
...
def _is_owned_hook_command(
    ...
    if len(tokens) != 5:
        return False
    executable, config_flag, config_value, state_flag, state_value = tokens
    return (
...
def _has_owned_hook(
    settings: dict[str, Any], config_path: Path, state_path: Path
) -> bool:
    return _owned_hook_command(settings, config_path, state_path) is not None
